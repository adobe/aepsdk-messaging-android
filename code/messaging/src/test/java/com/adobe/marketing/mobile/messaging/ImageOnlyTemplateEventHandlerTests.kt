...
class ImageOnlyTemplateEventHandlerTests {
    ...
    @Mock
    private lateinit var mockImageOnlyUI: ImageOnlyUI
    @Mock
    private lateinit var mockImageOnlyCardUIState: ImageOnlyCardUIState
    @Mock
    private lateinit var mockImageOnlyTemplate: ImageOnlyTemplate
    @Mock
    ...
    @BeforeTest
    fun setup() {
        mockImageOnlyUI = mock(ImageOnlyUI::class.java)
        mockImageOnlyCardUIState = mock(ImageOnlyCardUIState::class.java)
        mockImageOnlyTemplate = mock(ImageOnlyTemplate::class.java)
        ...
        `when`(mockImageOnlyTemplate.id).thenReturn("mockId")
        `when`(mockImageOnlyTemplate.getType()).thenReturn(AepUITemplateType.IMAGE_ONLY)
        `when`(mockImageOnlyUI.getTemplate()).thenReturn(mockImageOnlyTemplate)
        `when`(mockImageOnlyUI.getState()).thenReturn(mockImageOnlyCardUIState)
    ...
    @AfterTest
    fun tearDown() {
        reset(mockImageOnlyUI, mockImageOnlyTemplate, mockServiceProvider, mockUriOpening, mockContentCardMapper, mockContentCardSchemaData)
        clearAllMocks()
    }
    ...
    @Test
    fun `Image Only Template event handler receives a display event when card already displayed`() {
        runTest {
    ...
    @Test
    fun `Image Only Template event handler receives a dismiss event when card already dismissed`() {
        runTest {
    ...
    @Test
    fun `Image Only Template event handler receives a click event but callback on interact returns true`() {
        runTest {
            val callback = mock(ContentCardUIEventListener::class.java)
